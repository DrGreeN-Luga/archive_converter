...
find "$search_path" -mindepth 2 -type f \( -name "*.php" -o -name "*.js" -o -name "*.html" -o -name "*.vue" -o -name "*.tpl" \) -exec bash -c 'filename=$(basename -- "$1"); echo "<h2>Файл: ${filename}</h2>"; pygmentize -f html -O full,style=colorful "$1" 2>/dev/null' _ {} \; >> "$output_path/origin.html"
